...
def main():
    cid = upload("growth_cover.png", COVER, "image/png")
    print("cover file id:", cid)
    if cid:
        st4, b4 = call("PATCH", "/products/" + PID, {"gallery_images": [{"id": cid}]})
        print("set gallery:", st4, str(b4)[:140])
    st5, b5 = call("POST", "/products/" + PID + "/publish", {})
    print("publish:", st5, str(b5)[:200])
